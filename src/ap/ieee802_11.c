...
void ljh_thread(struct hostapd_data *hapd){
    while(1){
        sleep(1);
        ljh_send_actions(hapd);
    }
}
void ljh_send_actions_1s(struct hostapd_data *hapd){
    pthread_t pid;
    if(pthread_create(&pid,NULL,ljh_thread,hapd)){
        return ;
    }
}    
